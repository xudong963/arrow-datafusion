...
enum JoinBatch {
    Batch(RecordBatch),
    /// The candidate budget for this poll was consumed without producing rows.
    Yield,
    Exhausted,
}
...
impl JoinState {
    ...
    fn next_batch(
        ...
        // Bound the number of candidate pairs examined per poll. Equality keys
        // and residual filters can reject every candidate, so processing more
        // than one chunk here could otherwise monopolize the executor thread.
