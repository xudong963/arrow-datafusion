...
#[tokio::test]
async fn selective_filter_yields_after_each_candidate_batch() -> Result<()> {
    const BATCH_SIZE: usize = 7;
    const RIGHT_ROWS: usize = BATCH_SIZE * 3;
    ...
    let candidates_after_first_poll = join
        .metrics()
        .and_then(|metrics| metrics.sum_by_name("candidate_rows"))
        .map(|value| value.as_usize())
        .unwrap_or_default();
    assert_eq!(candidates_after_first_poll, BATCH_SIZE);

    let batches = tokio::time::timeout(Duration::from_secs(5), common::collect(stream))
        .await
        .expect("IEJoin must wake itself after yielding")?;
    assert!(batches.is_empty());
    let total_candidates = join
        .metrics()
        .and_then(|metrics| metrics.sum_by_name("candidate_rows"))
        .map(|value| value.as_usize())
        .unwrap_or_default();
    assert_eq!(total_candidates, RIGHT_ROWS);
    Ok(())
}
