...
fn vectorized_equal_to<GroupColumnBuilder: GroupColumn>(
    ...
    group.bench_function(id, |b| {
        ...
        b.iter(|| {
            // Rebuilding is a must as `vectorized_equal_to` will modify the
            // input buffer and without rebuilding all benchmarks after the
            // first one won't be meaningful.
            let mut equal_to_results_buffer =
                BooleanBufferBuilder::new(equal_to_results.len());
            for &value in &equal_to_results {
                equal_to_results_buffer.append(value);
            }
            builder.vectorized_equal_to(rows, input, rows, &mut equal_to_results_buffer);

            // Make sure that the compiler does not optimize away the call
            black_box(equal_to_results_buffer);
        });
    });
}
